  ...
  class Callbacks
    def self.around_create(record)
      ...
      if record.created_at.nil? || record.created_at >= now || record.created_at == record.updated_at || record.override_timestamps
        ...
        record.id = Yotsuba::Snowflake.id_at(record.created_at)
        tries = 0
  ...
  class << self
    ...
    def ensure_id_sequences_exist
      ...
      connection.tables.each do |table|
        ...
        # If we were on PostgreSQL 9.5+, we could do CREATE SEQUENCE IF
        # NOT EXISTS, but we can't depend on that. Instead, catch the
        # possible exception and ignore it. Note that `seq_name` isn't a
        # column name, but it's a relation, like a column, and follows
        # the same quoting rules in PostgreSQL.
        connection.execute(<<~SQL)
          DO $$
            BEGIN
